...
export default function InsurancePage() {
  ...
  const rowBg = (s: string) => s === 'EXPIRED' ? 'bg-[#DC2626]/10' : s === 'EXPIRING_SOON' ? 'bg-[#F59E0B]/10' : '';

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div><h2 className="font-['Oswald'] text-xl font-bold text-[#0D2847] tracking-wide uppercase">Insurance</h2><p className="font-['Rajdhani'] text-sm text-[#7A9AB8]">Vehicle insurance policies</p></div>
        <button onClick={() => { setForm({ type: 'COMPREHENSIVE', status: 'ACTIVE' }); setEditItem(null); setModalOpen(true); }} className="flex items-center gap-2 bg-[#1565C0] hover:bg-[#0D2847] text-white font-medium px-4 py-2.5 rounded-lg shadow-sm transition-colors">
          <Plus className="w-4 h-4" /> Add Policy
        </button>
      </div>

      {expiring?.length > 0 && (
        <div className="bg-[#F59E0B]/10 border-l-4 border-[#F59E0B] rounded-xl p-4 flex items-center gap-3">
          <AlertTriangle className="w-5 h-5 text-[#F59E0B] flex-shrink-0" />
          <p className="text-sm text-[#0D2847]"><strong>{expiring.length} insurance {expiring.length === 1 ? 'policy' : 'policies'}</strong> expiring within 30 days.</p>
          <Link href="#table" className="text-sm font-semibold text-[#1565C0] hover:underline">View →</Link>
        </div>
      )}

      <div className="bg-white rounded-xl border border-[#E0E8F0] shadow-sm p-4 mb-5 flex gap-3">
        <select value={status} onChange={e => { setStatus(e.target.value); setPage(1); }} className="h-10 w-48 rounded-lg border border-[#E0E8F0] text-sm text-[#0D2847]">
          <option value="">All Statuses</option>
          {I_STATUSES.map(s => <option key={s} value={s}>{s.replace(/_/g,' ')}</option>)}
        </select>
      </div>

      <div id="table" className="bg-white rounded-xl border border-[#E0E8F0] shadow-sm overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#F4F6F8] border-b border-[#E0E8F0]">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Provider</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Policy #</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Type</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Premium</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Cover</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Start</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Expiry</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Status</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[#1A4A7A]">Agent</th>
