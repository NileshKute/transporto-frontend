...
export default function WhatsAppPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold text-slate-100">WhatsApp Messages</h2>
      ...
      {isLoading ? <LoadingSpinner /> : !data?.data?.length ? (
        ...
        <div className="space-y-3">
          {data.data.map((msg: any) => {
            ...
            const Icon = ti.icon;
            return (
              <div key={msg.id} className="bg-[#111827] border border-[#1e293b] rounded-xl p-5 hover:border-[#334155] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold text-slate-200 text-sm">{msg.driver?.name || 'Unknown Driver'}</span>
                    ...
                    {/* Message */}
                    <p className="text-sm text-slate-300 bg-[#0a0e1a] border border-[#1e293b] rounded-lg p-3 mb-3 font-mono">"{msg.message}"</p>

                    {/* Parsed Data */}
                    {msg.parsedData && Object.keys(msg.parsedData).length > 0 && (
                      <div className="flex flex-wrap gap-3 mb-2">
                        {Object.entries(msg.parsedData).filter(([k, v]) => v != null).map(([k, v]) => (
                          <div key={k} className="bg-[#0a0e1a] border border-[#1e293b] rounded-lg px-3 py-1.5">
