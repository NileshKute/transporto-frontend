...
export default function ColdStoragePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-[var(--text-primary)]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Cold Storage</h2>
        <p className="text-sm text-[var(--text-secondary)]">Monitor temperature and storage units</p>
      </div>
        ...
        <div className="bg-red-50 border-l-4 border-red-500 rounded-xl p-4">
          ...
          <div className="space-y-2">
            {alerts.slice(0, 3).map((a: any) => (
              <div key={a.id} className="flex items-center justify-between text-sm text-[var(--text-primary)]">
          ...
          {units.map((u: any) => {
            ...
            return (
              <Link key={u.id} href={`/cold-storage/${u.id}`}>
                <div className="bg-white border border-[var(--border-light)] rounded-xl p-5 shadow-[var(--shadow-card)] hover:bg-[var(--bg-card-hover)] transition-all cursor-pointer">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <span className="text-xs bg-[var(--bg-table-header)] text-[var(--text-secondary)] px-2 py-0.5 rounded font-medium">{u.type?.replace(/_/g,' ')}</span>
                    </div>
                  ...
                  <div className="flex items-end justify-between mb-4">
                    <div>
                      <p className="text-xs text-[var(--text-muted)] mb-1">Current Temp</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-semibold text-[var(--text-secondary)]">{u.targetTemp != null && u.targetTemp > 0 ? '+' : ''}{u.targetTemp}°C</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="h-2 bg-[var(--bg-table-header)] rounded-full overflow-hidden">
