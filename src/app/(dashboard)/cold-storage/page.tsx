...
import { safe, safeNumber } from '@/lib/utils';
        ...
        <div className="bg-[#DC2626]/10 border-l-4 border-[#DC2626] rounded-xl p-4">
          ...
          <div className="space-y-2">
            {alerts.slice(0, 3).map((a: any) => (
              <div key={String(a.id ?? '')} className="flex items-center justify-between text-sm text-[#0D2847]">
                <span>
                  {typeof a.unit?.name === 'string' ? a.unit.name : safe(a.unit?.name)} —{' '}
                  {typeof a.alertType === 'string' ? a.alertType.replace(/_/g, ' ') : safe(a.alertType)}
                </span>
                <span className="text-xs font-mono">
                  {Number.isFinite(safeNumber(a.temperature, NaN)) ? `${safeNumber(a.temperature)}°C` : ''}
                </span>
              </div>
            ))}
          </div>
      ...
      {isLoading ? <LoadingSpinner /> : !units?.length ? <EmptyState message="No cold storage units" /> : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-5">
          {units.map((u: any) => {
            ...
            return (
              <Link key={String(u.id ?? '')} href={`/cold-storage/${u.id}`}>
                <div className="bg-white rounded-xl border border-[#E0E8F0] p-5 shadow-sm hover:shadow-md transition-all cursor-pointer">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <div className={`w-2.5 h-2.5 rounded-full ${statusDot(String(u.status ?? ''))} ${u.status === 'WARNING' || u.status === 'CRITICAL' ? 'animate-pulse' : ''}`} />
                        <h3 className="font-semibold text-[#0D2847]">{safe(u.name)}</h3>
                      </div>
                      <span className="text-xs bg-[#F4F6F8] text-[#1A4A7A] px-2 py-0.5 rounded font-medium">
                        {typeof u.type === 'string' ? u.type.replace(/_/g, ' ') : safe(u.type)}
                      </span>
                    </div>
                  ...
                  <div className="flex items-end justify-between mb-4">
                    <div>
                      ...
                      <p className={`text-4xl font-bold font-mono ${tempColor(String(u.status ?? ''))}`}>
                        {Number.isFinite(safeNumber(latestTemp, NaN))
                          ? `${safeNumber(latestTemp) > 0 ? '+' : ''}${safeNumber(latestTemp)}°`
                          : '—'}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-[#1A4A7A]">Target</p>
                      <p className="text-sm font-semibold text-[#1A4A7A]">
                        {Number.isFinite(safeNumber(u.targetTemp, NaN)) ? `${safeNumber(u.targetTemp) > 0 ? '+' : ''}${safeNumber(u.targetTemp)}°C` : '—'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between text-xs text-[#1A4A7A] mb-1.5">
                      ...
                      <span className="font-mono">
                        {capacityUsed} / {Number.isFinite(safeNumber(u.capacityTotal, NaN)) ? safeNumber(u.capacityTotal) : '—'}{' '}
                        {typeof u.capacityUnit === 'string' && u.capacityUnit ? u.capacityUnit : 'Tons'}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-[#1A4A7A]">
                    ...
                    <div className="flex items-center gap-1"><Thermometer className="w-3 h-3" /> Sensor: {u.sensorId != null && u.sensorId !== '' ? safe(u.sensorId) : 'N/A'}</div>
                  </div>
                </div>
              </Link>
            );
