  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <p className="font-['Rajdhani'] text-sm text-[#7A9AB8] mt-0.5">Manage all trips</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-[#E0E8F0] shadow-sm p-4 mb-5 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search trip#, location, client..." className="h-10 pl-10 rounded-lg border border-[#E0E8F0] text-sm text-[#0D2847] w-full" />
        </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="New Trip" size="lg">
        <div className="p-5 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={() => createMutation.mutate(form)} disabled={createMutation.isPending} className="flex-1 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-[#F4F6F8]0 disabled:opacity-50 rounded-lg">
      ...
      <Modal isOpen={!!completeTrip} onClose={() => setCompleteTrip(null)} title="Complete Trip" size="sm">
        <div className="p-5 space-y-4">
          <p className="text-sm text-[#7A9AB8]">Trip <span className="text-blue-400 font-mono">{completeTrip?.tripNumber}</span></p>
          <div>
