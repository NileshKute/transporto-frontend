...
function parseDrivers(raw: unknown): { id: string; name: string; nickname?: string | null }[] {
  const arr = Array.isArray(raw) ? raw : [];
  return arr
    .map((d: Record<string, unknown>) => ({
      id: String(d.id ?? ''),
      name: String(d.name ?? d.fullName ?? '').trim(),
      nickname: d.nickname != null ? String(d.nickname).trim() || null : null,
    }))
...
export function DailyTripLogPanel() {
                      ...
                      <div className="flex-1 min-w-[120px] lg:max-w-[200px]">
                        <LocationSelect
                          ...
                          onChange={(name) =>
                            updateBlock(block.id, (b) => ({
                              ...b,
                              trips: b.trips.map((t, j) => (j === ti ? { ...t, from: name } : t)),
                            }))
                          }
                          placeholder="From"
                          inputClassName="h-9"
                          ...
                          onChange={(name, picked) =>
                            updateBlock(block.id, (b) => ({
                              ...
                              trips: b.trips.map((t, j) => {
                                if (j !== ti) return t;
                                let next: TripLine = { ...t, to: name };
                                if (picked?.type === 'CLIENT') {
                                  const c = (picked.shortName || picked.name).trim();
                                  next = onClientPick({ ...next, client: c }, c);
                                }
                                return next;
                              }),
                            }))
