...
const statusConfig: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  PENDING_VERIFICATION: { bg: 'bg-amber-100', text: 'text-amber-900', dot: 'bg-amber-500' },
...
export function StatusBadge({ status, size = 'md' }: StatusBadgeProps) {
  ...
  if (s === 'PENDING_VERIFICATION') {
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full font-medium border border-amber-300 bg-amber-100 text-amber-950 font-['Barlow_Condensed'] uppercase tracking-wide ${sizeClass}`}
      >
  ...
  if (s === 'REJECTED') {
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full font-medium border border-red-200 bg-red-100 text-red-800 font-['Barlow_Condensed'] uppercase tracking-wide ${sizeClass}`}
      >
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full font-medium border border-transparent font-['Barlow_Condensed'] ${config.bg} ${config.text} ${sizeClass}`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${config.dot} ${s === 'WARNING' || s === 'CRITICAL' ? 'animate-pulse' : ''}`}
      />
      {s.replace(/_/g, ' ')}
    </span>
  );
